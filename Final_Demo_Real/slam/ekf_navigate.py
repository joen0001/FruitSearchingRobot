...
class EKF:
    ...
    def predict(self, raw_drive_meas):
        ...
        # print("[predict] Current state, x = %f"%x[0], "y = %f"%x[1], "theta = %f degrees" %float((x[2])*180/np.pi))
    ...
    def update(self, measurements):
        ...
        #     print("[update] Marker location, x = %f"%self.taglist[0], "y = %f"%self.taglist[1])
        # Get state estimate
        self.set_state_vector(x)
        # Correct covariance
        self.P = (np.eye(x.shape[0]) - K @ H) @ self.P
        
        state = np.array(self.get_state_vector()).flatten()
        # # Drive and predict
