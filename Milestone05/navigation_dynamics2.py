...
def drive_to_point(waypoint, robot_pose):
    ...
    if theta_des >= 0:
        lower_bound = theta_des - 0.05
        print("lower_bound: ", lower_bound*180/np.pi)
        upper_bound = theta_des + 0.05
        print("upper_bound: ", upper_bound*180/np.pi)
        while theta_robot < lower_bound or theta_robot > upper_bound:

            lv, rv = operate.pibot.set_velocity([0,1], turning_tick=wheel_vel, time=0.05)
            drive_meas = measure.Drive(lv,rv,0.05)
    ...
    #dist_est = np.sqrt(x_dif**2 + y_dif**2)
 
    #x_lower_bound = x_des - 0.05
    #x_upper_bound = x_des + 0.05
    #y_lower_bound = y_des - 0.05
    #y_upper_bound = y_des + 0.05

    #while x_robot < x_lower_bound or x_robot > x_upper_bound or y_robot < y_lower_bound or y_robot > y_upper_bound:
        #if theta_des >= 0:
        #    lv, rv = operate.pibot.set_velocity([0,1], turning_tick=wheel_vel, time=0.05)
        #else :
        #    lv, rv = operate.pibot.set_velocity([0,-1], turning_tick=wheel_vel, time=0.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--map", type=str, default='true_map.txt') #change this line depending on what map to read
    ...
    while True:
        ...
        #print("pre movement pose: ", robot_pose)
        robot_pose = get_robot_pose()
        print("pre movement pose: ", robot_pose)
