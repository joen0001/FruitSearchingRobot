...
def drive_to_point(waypoint, robot_pose):
    ...
    while np.sqrt(x_dif**2 + y_dif**2)>0.1:
        ...
        start = time.time()
        total_time = 0
        while total_time < drive_time/10: 
            end = time.time()
...
if __name__ == "__main__":
    ...
    i = 0
    fruit_list, fruit_true_pos, aruco_true_pos = read_true_map("M4_true_map_3fruits.txt")
    ...
    for current_goal in range(len(search_list)):
        ...
        #print("without 0")
       #print(path2)
        waypoints = path2 #[[0.5, 0.5], [1, 0], [1, 0.75], [1, -0.8]]
