    ...
    def record_data(self):
        ...
        if self.command['save_inference']:
            if self.file_output is not None:
                ...
                print(self.file_output[0])
                    
                content = str(np.asarray(self.file_output[0]))
                print(content)
                file.write(content)
                file.write('\n')
                
                #fp = os.path.join(self.folder, f'img_{self.image_id}.png') #LISA
                cv2.imwrite(operate.pred_fname, self.network_vis) #LISA
                
                #np.savetxt(r'np.txt', df.values, fmt='%d') #LISA delete
    ...
    def update_keyboard(self):
        for event in pygame.event.get():
            ...
            # turn left
                ...
                self.command['motion'] = [0, 3] # TODO: replace with your M1 code to make the robot turn left
            # drive right
            elif event.type == pygame.KEYDOWN and event.key == pygame.K_RIGHT:
                self.command['motion'] = [0, -3] # TODO: replace with your M1 code to make the robot turn right
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", default='network/scripts/model/best100.pt')
